window.addEventListener('DOMContentLoaded', () => {
    ...
    // Trip amount Calculator
    let personAmount = document.querySelectorAll('.counter-block-input')[0],
        daysAmount = document.querySelectorAll('.counter-block-input')[1],
        tripAmount = document.getElementById('total'),
        restingPlase = document.getElementById('select'),
        totalPrise = 0,
        persons = 0,
        days = 0;

        tripAmount.innerHTML = 0;

        personAmount.addEventListener('change', function() {
            persons = +this.value;
            if(days == 0 || persons == 0) {
                tripAmount.innerHTML = 0;
            } else {
                totalPrise = (days + persons)*4000;
                tripAmount.innerHTML = totalPrise;
            }
        });
        daysAmount.addEventListener('change', function() {
            days = +this.value;
            if(persons == 0 || days == 0) {
                tripAmount.innerHTML = 0;
            } else {
        ...
        restingPlase.addEventListener('change', function() {
            if(days == 0 || persons == 0) {
                tripAmount.innerHTML = 0;
            } else {
                let a = totalPrise;
                tripAmount.innerHTML = a * this.options[this.selectedIndex].value;
            }
        });
});